...
interface EditPayrollScheduleFormProps {
  payrollSchedule: EmployeePaySchedule;
  onCancel?: () => void;
  onSubmit?: (data: EmployeePaySchedule) => Promise<void>;
}

export const EditPayrollScheduleForm: React.FC<EditPayrollScheduleFormProps> = ({ 
  payrollSchedule,
  onCancel, 
  onSubmit 
}) => {
  ...
  const form = useForm<PayrollScheduleFormValues>({
    ...
    defaultValues: {
      payFrequency: payrollSchedule.payFrequency,
      nextPayDate: formatDateForInput(payrollSchedule.nextPayDate),
      taxes: payrollSchedule.taxes.toString(),
      isDefault: payrollSchedule.isDefault,
    },
  });
  ...
  const handleSubmit = async (data: PayrollScheduleFormValues) => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Payroll schedule updated successfully.",
      });
      ...
      toast({
        title: "Error",
        description: "Failed to update payroll schedule. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
...
export default EditPayrollScheduleForm;