...
import { CheckCircle, Save, ArrowLeft, PackageIcon } from "lucide-react";
import {
  Form,
  FormControl,
  FormDescription,
...
const formSchema = z.object({
  ...
  warehouseId: z.string().min(1, { message: "Warehouse is required" }).optional(),
  categoryId: z.string().min(1, { message: "Category is required" }).optional(),
  description: z.string().optional(),
  purchasePrice: z.coerce.number().min(0, { message: "Price must be at least 0" }),
...
const NewProductPage: React.FC = () => {
  ...
  const { categories, warehouses, isLoading: isLoadingInitials, isError } = useProductInitials();
  const { createProduct, error, isLoading: isCreatingProduct } = useProductsStore();
  ...
  const onSubmit = async (values: FormData) => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `${productType === "product" ? "Product" : "Service"} created successfully`,
      });
  ...
  const isLoading = isSubmitting || isCreatingProduct;
    ...
    <div className="container mx-auto py-8 space-y-6 px-4 sm:px-6">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent border-l-4 border-emerald-500 pl-2 flex gap-4 justify-start items-center">
                ...
                >
                  {["product", "service"].map((type) => (
                    ...
                    >
                      ...
                      <FormLabel htmlFor={type} className="cursor-pointer font-medium">
...
const GeneralInfoSection: React.FC<{
  ...
  >
    <Card className="overflow-hidden border-l-4 border-l-emerald-500/70 shadow-sm hover:shadow-md transition-shadow">
      <CardHeader className="bg-muted/30">
        <CardTitle>{t("sections.generalInfo.title")}</CardTitle>
        <CardDescription>
          {t.raw("sections.generalInfo.description").replace(
            "{productType}",
      ...
      <CardContent className="pt-6">
        <div className="grid gap-6 md:grid-cols-2">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="font-medium">{t("fields.name")}</FormLabel>
                <FormControl>
                  <Input
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="font-medium">{t("fields.category")}</FormLabel>
                <Select
                  onValueChange={field.onChange}
                  defaultValue={field.value}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="font-medium">{t("fields.barcode")}</FormLabel>
                <FormControl>
                  <Input
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="col-span-2">
                <FormLabel className="font-medium">{t("fields.description")}</FormLabel>
                <FormControl>
                  <Textarea
...
const InventorySection: React.FC<{
  ...
  warehouses: any[];
  isLoadingInitials: boolean;
}> = ({ form, t, variants, warehouses, isLoadingInitials }) => (
  <motion.div
    initial="hidden"
    animate="visible"
    variants={variants}
  ...
  >
    <Card className="overflow-hidden border-l-4 border-l-emerald-500/60 shadow-sm hover:shadow-md transition-shadow">
      <CardHeader className="bg-muted/30">
        <CardTitle>{t("sections.inventory.title")}</CardTitle>
        <CardDescription>{t("sections.inventory.description")}</CardDescription>
      </CardHeader>
      <CardContent className="pt-6">
        <div className="grid gap-6 md:grid-cols-2">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="font-medium">
                  {t("fields.initialQuantity.label")}
                </FormLabel>
                <FormControl>
                  <Input
                    type="number"
                ...
                <FormDescription>
                  {t("fields.initialQuantity.description")}
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="font-medium">{t("fields.warehouse")}</FormLabel>
                <Select
                  onValueChange={field.onChange}
                  defaultValue={field.value}
                  disabled={isLoadingInitials}
...
const TaxInfoSection: React.FC<{
  ...
  >
    <Card className="overflow-hidden border-l-4 border-l-emerald-500/50 shadow-sm hover:shadow-md transition-shadow">
      <CardHeader className="bg-muted/30">
        <CardTitle>{t("sections.taxInfo.title")}</CardTitle>
        <CardDescription>
          {t.raw("sections.taxInfo.description").replace(
            "{productType}",
            t(`fields.productType.${productType}`)
          )}
        </CardDescription>
      ...
      <CardContent className="pt-6">
        <div className="grid gap-6 md:grid-cols-2">
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="space-y-3">
                <FormLabel className="font-medium">{t("fields.taxType.label")}</FormLabel>
                <FormControl>
                  <RadioGroup
                    onValueChange={field.onChange}
                    defaultValue={field.value}
                  ...
                  >
                    {["taxable", "non_taxable"].map((type) => (
                      ...
                      >
                        <RadioGroupItem value={type} id={type} />
                        <FormLabel htmlFor={type} className="cursor-pointer font-medium">
                          {t(`fields.taxType.${type === "taxable" ? "taxable" : "nonTaxable"}`)}
                        </FormLabel>
                      </div>
                    ))}
                  </RadioGroup>
...
const PricingSection: React.FC<{
  ...
  >
    <Card className="overflow-hidden border-l-4 border-l-emerald-500/40 shadow-sm hover:shadow-md transition-shadow">
      <CardHeader className="bg-muted/30">
        <CardTitle>{t("sections.pricing.title")}</CardTitle>
        <CardDescription>
          {t.raw("sections.pricing.description").replace(
            "{productType}",
            t(`fields.productType.${productType}`)
          )}
        </CardDescription>
          ...
          {["purchasePrice", "salePrice"].map((priceType) => (
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="font-medium">
                    {t(`fields.${priceType === "salePrice" ? "salesPrice" : priceType}`)}
                  </FormLabel>
...
const SubmitSection: React.FC<{
  ...
  productType: string;
  onCancel: () => void;
}> = ({ t, isLoading, formSubmitted, productType, onCancel }) => (
  <motion.div
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
