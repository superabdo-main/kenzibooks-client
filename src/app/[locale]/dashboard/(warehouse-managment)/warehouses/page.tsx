...
import { WarehouseIcon, PlusIcon } from "lucide-react";
import { useWarehousesStore } from "@/stores/warehouses.store";
import { Skeleton } from "@/components/shadcn-ui/skeleton";
import { Button } from "@/components/shadcn-ui/button";
import Link from "next/link";
import { exportWarehousesToCSV, exportWarehousesToExcel, exportWarehousesToPDF } from "@/lib/export/warehouses";
...
const WarehousesPage = () => {
  ...
  const { deleteWarehouse } = useWarehousesStore();
  ...
  const columns = createColumns(t, handleDeleteWarehouse);
        ...
        <CardHeader className="bg-muted/30 py-5 border-b border-border flex w-full justify-between flex-row">
          ...
          <div>
            <Button
              ...
              asChild
            >
              <Link href={"/dashboard/warehouses/new-warehouse"}>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <DataTable
              ...
              onExportPDF={() => exportWarehousesToPDF(warehouses || [], t)}
              onExportExcel={() => exportWarehousesToExcel(warehouses || [], t)}
              onExportCSV={() => exportWarehousesToCSV(warehouses || [], t)}
            />
          )}
        </CardContent>
      </Card>
