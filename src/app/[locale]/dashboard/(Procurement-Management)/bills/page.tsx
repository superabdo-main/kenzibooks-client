...
import { exportBillsToCSV, exportBillsToExcel, exportBillsToPDF } from "@/lib/export/bills";
...
const BillsPage = () => {
  ...
  const {purchases, isLoading, mutate} = usePurchases({type: 'BILL'})
  ...
  const columns = createColumns(t, handleDeleteBill);
        ...
        <CardHeader className="bg-muted/30 py-5 border-b border-border flex w-full justify-between flex-row">
          ...
          <div>
            <Button
              ...
              asChild
            >
              <Link href={`/dashboard/bills/new-bill`}>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <DataTable
              ...
              onExportPDF={() => exportBillsToPDF(purchases || [], t)}
              onExportExcel={() => exportBillsToExcel(purchases || [], t)}
              onExportCSV={() => exportBillsToCSV(purchases || [], t)}
            />
          )}
        </CardContent>
      </Card>
