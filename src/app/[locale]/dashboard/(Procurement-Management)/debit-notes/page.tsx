...
const DebitNotesPage = () => {
  ...
  const { purchases, isLoading, mutate } = usePurchases({ type: "DEBIT_NOTE" });
  const { session } = useAuth();
  const { toast } = useToast();
  const { deletePurchaseById } = usePurchasesStore();
  ...
  const handleDeletePurchase = async (id: string) => {
    ...
    toast({
      ...
      description: "The debit note has been deleted successfully.",
      variant: "default",
    });
    mutate();
  };
        ...
        <CardHeader className="bg-muted/30 py-5 border-b border-border flex w-full justify-between flex-row">
          ...
          <div>
            <Button
              ...
              asChild
            >
              <Link href={`/dashboard/debit-notes/new-debit-note`}>
