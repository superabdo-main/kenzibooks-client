...
import { exportPurchasesToCSV, exportPurchasesToExcel, exportPurchasesToPDF } from "@/lib/export/purchases";
import { usePurchases } from "@/features/purchases/hooks/usePurchases";
import { useAuth } from "@/contexts/AuthContext";
import { useToast } from "@/hooks/use-toast";

const PurchaseEstimatePage = () => {
  const t = useTranslations("Purchases");
  const {purchases, isLoading, mutate} = usePurchases({type: 'PURCHASE_ESTIMATES'})
  ...
  const handleDeletePurchase = async (id: string) => {
    ...
    const response = await deletePurchaseById({
  ...
  const columns = createColumns(t, handleDeletePurchase);
        ...
        <CardHeader className="bg-muted/30 py-5 border-b border-border flex w-full justify-between flex-row">
          ...
          <div>
            <Button
              ...
              asChild
            >
              <Link href={`/dashboard/purchase-estimate/new-purchase`}>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <DataTable
              columns={columns}
              data={purchases || []}
              searchColumn="supplierEmail"
              translations={tableTranslations}
              onExportPDF={() => exportPurchasesToPDF(purchases || [], t)}
              onExportExcel={() => exportPurchasesToExcel(purchases || [], t)}
              onExportCSV={() => exportPurchasesToCSV(purchases || [], t)}
            />
          )}
        </CardContent>
      </Card>
...
export default PurchaseEstimatePage; 